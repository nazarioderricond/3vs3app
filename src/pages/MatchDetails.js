...
export async function renderMatchDetailsPage(params) {
  ...
  const homeScorers = scorers?.filter(s => String(s.team_id) === String(match.home_team_id) && s.goals > 0) || [];
  const awayScorers = scorers?.filter(s => String(s.team_id) === String(match.away_team_id) && s.goals > 0) || [];
