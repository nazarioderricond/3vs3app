        ...
        <table class="w-100" style="width: 100%; border-collapse: collapse;">
          <thead>
            <tr style="border-bottom: 2px solid var(--color-yellow);">
              <th class="text-left p-sm">Utente</th>
              <th class="text-left p-sm">Email</th>
              <th class="text-center p-sm">Ruolo</th>
              <th class="text-right p-sm">Azioni</th>
            </tr>
          </thead>
      ...
      <tr style="border-bottom: 1px solid rgba(255,255,255,0.1);">
        <td class="p-sm">
          <div style="display: flex; align-items: center; gap: 0.5rem;">
            ${user.profile_image_url ?
                `<img src="${user.profile_image_url}" style="width: 30px; height: 30px; border-radius: 50%; object-fit: cover;">` :
                `<div style="width: 30px; height: 30px; border-radius: 50%; background: var(--color-yellow); color: black; display: flex; align-items: center; justify-content: center; font-weight: bold;">${user.first_name[0]}</div>`
            }
            ${user.first_name} ${user.last_name}
          </div>
        </td>
        <td class="p-sm" style="opacity: 0.8;">${user.email || 'N/A'}</td>
        <td class="text-center p-sm">
        ...
        <td class="text-right p-sm">
          <button class="btn btn-sm role-toggle-btn" 
            ...
            style="font-size: 0.8rem; padding: 0.25rem 0.5rem; border: 1px solid var(--color-yellow); background: transparent; color: var(--color-yellow); cursor: pointer;">
